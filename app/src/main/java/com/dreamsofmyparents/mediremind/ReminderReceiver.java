...
public class ReminderReceiver extends BroadcastReceiver {
    ...
    private void acquireWakeLock(Context context) {
        try {
            ...
            if (powerManager != null && wakeLock == null) {
                wakeLock = powerManager.newWakeLock(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON | PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "MediRemind:AlarmWakeLock");
    ...
    private void createAlarmNotification(Context context, String medName, String dose, String meal, int reminderId) {
        ...
        PendingIntent alarmPendingIntent = PendingIntent.getActivity(context, reminderId, alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        ...
        PendingIntent acceptPendingIntent = PendingIntent.getBroadcast(context, 1, acceptIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        ...
        PendingIntent declinePendingIntent = PendingIntent.getBroadcast(context, 2, declineIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        String notificationText = medName + " (" + dose + ") " + meal + " খাওয়ার সময় হয়েছে";

        // Build the notification
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, channelId).setSmallIcon(R.drawable.ic_medication).setContentTitle("🚨 ওষুধের এলার্ম").setContentText(notificationText).setStyle(new NotificationCompat.BigTextStyle().bigText(notificationText)).setPriority(NotificationCompat.PRIORITY_HIGH).setCategory(NotificationCompat.CATEGORY_ALARM).setAutoCancel(true).setContentIntent(alarmPendingIntent).addAction(R.drawable.ic_medication, "নিয়েছি ✓", acceptPendingIntent).addAction(R.drawable.ic_settings, "পরে নেব", declinePendingIntent).setDefaults(NotificationCompat.DEFAULT_ALL).setVisibility(NotificationCompat.VISIBILITY_PUBLIC);
